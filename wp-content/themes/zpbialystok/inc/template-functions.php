 ...
 * @return array
 */
function zp_bialystok_body_classes( $classes ) {
...
add_filter( 'body_class', 'zp_bialystok_body_classes' );
 ...
 * @return array
 */
function zp_bialystok_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'zp_bialystok_post_classes', 10, 3 );
 ...
 * @return void
 */
function zp_bialystok_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'zp_bialystok_pingback_header' );
 ...
 * @return void
 */
function zp_bialystok_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'zp_bialystok_supports_js' );
 ...
 * @return array
 */
function zp_bialystok_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'zp_bialystok_comment_form_defaults' );
 ...
 * @return bool
 */
function zp_bialystok_can_show_post_thumbnail() {
	return apply_filters(
		'zp_bialystok_can_show_post_thumbnail',
 ...
 * @return int
 */
function zp_bialystok_get_avatar_size() {
	return 60;
}
 ...
 */
function zp_bialystok_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'zpbialystok' ),
 ...
 */
function zp_bialystok_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . zp_bialystok_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'zp_bialystok_continue_reading_link_excerpt' );

/**
 * Create the continue reading link.
 */
function zp_bialystok_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . zp_bialystok_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'zp_bialystok_continue_reading_link' );

if ( ! function_exists( 'zp_bialystok_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function zp_bialystok_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'zpbialystok' ) : $title;
	}
}
add_filter( 'the_title', 'zp_bialystok_post_title' );
 ...
 * @return string
 */
function zp_bialystok_get_icon_svg( $group, $icon, $size = 24 ) {
	return Zp_Bialystok_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function zp_bialystok_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? zp_bialystok_get_icon_svg( 'ui', 'arrow_right' ) : zp_bialystok_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? zp_bialystok_get_icon_svg( 'ui', 'arrow_left' ) : zp_bialystok_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'zp_bialystok_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function zp_bialystok_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'zp_bialystok_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'zp_bialystok_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'zp_bialystok_generate_css' ) ) {
	...
	return zp_bialystok_generate_css( // @phpstan-ignore-line.
 ...
 */
function zp_bialystok_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function zp_bialystok_password_form( $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'zpbialystok' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'zpbialystok' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'zpbialystok' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'zpbialystok' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'zp_bialystok_password_form' );
 ...
 * @return array
 */
function zp_bialystok_get_attachment_image_attributes( $attr, $attachment, $size ) {
