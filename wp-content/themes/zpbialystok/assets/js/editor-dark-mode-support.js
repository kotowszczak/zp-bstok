/* global zpbialystokIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'zpbialystok-supports-dark-theme' ) ) {
	zpbialystokDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function zpbialystokDarkModeEditorInit( attempt ) {
	...
	if ( zpbialystokIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						zpbialystokDarkModeEditorInit( attempt + 1 );
					},
